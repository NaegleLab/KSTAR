...
def build_filtered_experiment(experiment, compendia, filtered_compendia, num_random_experiments, name ,selection_type='KSTAR_NUM_COMPENDIA_CLASS'):
    """Builds filtered experiment dataframe and saves the results as a tsv file.
    Random experiments are generated by sampling the same number of sites per compendia class as there are in the provided experiment dataframe

    Args:
        experiment (pandas DataFrame): filtered experiment to use for building random experiments
        compendia (pandas DataFrame): full compendia to use
        filtered_compendia (dict): compendia separated out into compendia sizes 
        num_random_experiments (int): number of random experiments to generate
        name (string): name of experiment or data column
        selection_type (str, optional): how to select the compendia. Defaults to 'KSTAR_NUM_COMPENDIA_CLASS'.

    Returns:
        pandas DataFrame: random experiments genrated
    """
...
def build_random_experiments_single_data_column(binary_evidence, compendia, num_random_experiments, phosphorylation_event, data_column, selection_type='KSTAR_NUM_COMPENDIA_CLASS'):
    ...
    random_experiments = build_filtered_experiment(experiment, compendia, filtered_compendia, num_random_experiments, data_column)
