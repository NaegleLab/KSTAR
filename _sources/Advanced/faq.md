...
If you do not see your question here, feel free to ask your question with this [form](https://forms.office.com/r/nJTYjhZ94L)
...
**Q: When should you use the evidence_size parameter instead of the threshold**

A: In general, if your data involves a direct comparison between samples (i.e. perturbations resulting in a fold change), using a threshold is generally recommended, as it provides a very clean interpretation of the resulting activity predictions. For example, if you have log fold changes and use a threshold of 1, you are predicting activity based on sites that increased 2-fold upon perturbation. However, if your data does not have clear comparisons and the "meaning" behind the threshold is less clear (such as for single tissue samples), it may be advantageous to simply define the number of sites to use. This avoids setting an arbitrary quantification threshold and accounts for any variability in the total number of sites measured in each sample.
...
**Q: What is the required number of random experiments, and why is 150 the default?**
...
**Q: How do you interpret the predictions when greater = False? Does it calculate both active and inactive kinase activity**

By setting greater = False, KSTAR will use sites with values less than the provided threshold, or sites with decreasing phosphorylation in your sample. As a result, KSTAR results are based on statistical enrichment of kinase substrates among sites with decreasing phosphorylation, and therefore should be viewed as kinases with significantly decreasing activity. So, let's say you have samples that were treated erlotinib, and EGFR inhibitor. If you set greater = False and obtain a statistically significant activity prediction for EGFR, this means that EGFR substrates are enriched among sites with decreasing phosphorylation and therefore EGFR has lower activity upon inhibitor treatment. This won't really indicate kinases with increasing activity upon treatment, however.

Note, you can also set greater =False when using the evidence size parameter, which will grab the N sites with the lowest quantification. This is generally not recommended, however, as it is more difficult to interpret what this means.