Options for Running KSTAR
=========================

.. container:: sphinx-features

   .. admonition:: On Galaxy Web Server
      :class: sphinx-feature

      Run KSTAR without any coding necessary using the `Galaxy web server`_. This is the recommended option for non-coding users, but has fewer options for downstream analysis. See the :ref:`linked tutorial<kstar-galaxy>` for more details.

   .. admonition:: In Python
      :class: sphinx-feature

      Run KSTAR with greater flexibility in :ref:`Python <python-quickstart>`. This is the recommended option for users familiar with Python and wanting to perform custom downstream analysis on their results.



.. _Galaxy web server: https://usegalaxy.org/