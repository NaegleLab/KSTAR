...
def parse_args():
    ...
    parser.add_argument('-p','--pevent','--phosphorylation_events', action = 'store', dest='pevent', help ='phosphorylation event type', choices=['Y','ST', 'STY'], default='STY', nargs='*')
    ...
    parser.add_argument('-a', '--arg', '--activity_arg', action='store', dest='activity_arg', help = 'activity arg to use', default='count', choices =['count','mean'])
...
def process_args(results):
    ...
    return experiment, log, data_columns, results.pevent, results.activity_arg, results.threshold

def run_kstar_analysis(experiment, log, networks, data_columns, activity_arg, threshold):
    kinact = kstar.KinaseActivity(experiment, log)
    kinact.add_networks_batch(networks)
    kinact.calculate_kinase_activities(data_columns, agg=activity_arg, threshold=threshold)
